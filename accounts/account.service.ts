...
async function sendVerificationEmail(account: any, origin: any){
    ...
    if (origin){
        ...
        message = `<p>Please use the below token to verify your emial address with the <code>/account/verify-email</code> api route:</p>
        <p><code>${account.verificationToken}</code></p>`;
    }

    console.log(`\n==================================================`);
    console.log(`[EMAIL DISPATCH] Verification link for ${account.email}:`);
    console.log(origin ? `${origin}/account/verify-email?token=${account.verificationToken}` : account.verificationToken);
    console.log(`==================================================\n`);
    ...
    try {
        ...
        console.error(`[Resend Error] Failed to send email to ${account.email}: ${err.message}`);
    }
}
...
async function sendAlreadyRegisteredEmail(email: any, origin: any){
    ...
    console.log(`\n==================================================`);
    console.log(`[EMAIL DISPATCH] Email ${email} is already registered!`);
    console.log(`==================================================\n`);
    ...
    try {
        await sendEmail({
            ...
            html: `<h4>Your Email <strong>${email}</strong> is already Registered.</h4>
            ${message}`
        });
    } catch (err: any) {
        console.error(`[Resend Error] Failed to send already registered email to ${email}: ${err.message}`);
    }
}
...
async function sendPasswordResetEmail(account: any, origin: any){
    ...
    if (origin){
        message = `<p> Please click the below link to reset your password, the link will be valid for 1 days:</p>
    ...
    console.log(`\n==================================================`);
    console.log(`[EMAIL DISPATCH] Password Reset link for ${account.email}:`);
    console.log(origin ? `${origin}/account/reset-password?token=${account.resetToken}` : account.resetToken);
    console.log(`==================================================\n`);
    ...
    try {
        ...
        console.error(`[Resend Error] Failed to send password reset email to ${account.email}: ${err.message}`);
    }
}